...
class PRI(BaseEstimator, ClusterMixin, TransformerMixin):
    ...
    def __init__(self, lambda_=2, sigma_initial=30, ayota=1, max_iter=1000, tol=5E-7, reduction_=False, n_groups=3, namedb=None, method='FP', optimization = None):
    ...
    def convergence(self, X, J, D, i, Da = None, Ja = None, BestX = None, bestJ = None, Patient_C = None, pos = None):
        ...
        if i > 0:
            ...
            if (J <= bestJ):
                ...
                pos = pos
                if Patient_C == int(0.4 * self.niter) or d < self.tole:
                    if d < self.tole:
                        print("llegue", d)
                        BestX = X
                        pos = i
    ...
    def get_params(self, deep=True):
        ...
        return {"lambda_": self.alpha, "sigma_initial": self.xsigma, "ayota": self.yota, "method": self.method, 'optimization': self.optimization}
...
class MiniBatchPRI(BaseEstimator, ClusterMixin, TransformerMixin):
    def __init__(self, lambda_=2, sigma_initial=30, ayota=1, max_epochs=300, tol=5E-8, reduction_=False, n_groups=3, namedb=None, minibatch_size=16, optimization = None):
    ...
    def convergence(self, X, Xo, sigma, i, Da = None, Ja = None, BestX = None, bestJ = None, Patient_C = None, pos = None):
        ...
        if i > 0:
            ...
            if (J <= bestJ):
                ...
                pos = pos
                if Patient_C == int(0.4 * self.niter) or stopc < self.tole:
                    if stopc < self.tole:
                        print("llegue", stopc)
                        BestX = X
                        pos = i
    ...
    def get_params(self, deep=True):
        ...
        return {"lambda_": self.alpha, "sigma_initial": self.xsigma, "ayota": self.yota, 'minibatch_size': self.minibatch_size, 'optimization': self.optimization, }
...
# p = MiniBatchPRI(n_groups = 3, lambda_ = 25,
#                 sigma_initial = 70, reduction_ = False, optimization = 'Nadam')
data = sio.loadmat('DB.mat')['DB'][0, 0]
be_2 = data['happy']
p.fit(be_2)
p.results()
